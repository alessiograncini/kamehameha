...
export class Line3D extends BaseScriptComponent {
  ...
  /**
   * Update the line with an array of world positions
   * This method allows external systems to dynamically update the line path
   */
  public updateWithPositions(positions: vec3[]): void {
    if (!positions || positions.length < 2) {
      print("Line3D: updateWithPositions requires at least 2 positions");
      return;
    }

    print("Line3D: updateWithPositions called with " + positions.length + " positions");

    // Ensure mesh visual is set up
    if (!this.meshVisual) {
      print("Line3D: Setting up mesh visual...");
      this.setupMeshVisual();
    }

    // Create temporary scene objects for the positions if pathPoints don't exist
    // or update existing pathPoints positions
    if (!this.pathPoints || this.pathPoints.length !== positions.length) {
      print("Line3D: Creating temporary path points for " + positions.length + " positions");
      
      // Clear existing path points if they exist
      if (this.pathPoints) {
      ...
      // Create new temporary scene objects for each position
      this.pathPoints = positions.map((position, index) => {
        const tempObject = global.scene.createSceneObject("TempLinePoint_" + index);
        tempObject.getTransform().setWorldPosition(position);
        print(`Line3D: Created temp point ${index} at (${position.x.toFixed(2)}, ${position.y.toFixed(2)}, ${position.z.toFixed(2)})`);
        return tempObject;
      });
    } else {
      // Update existing path points positions
      for (let i = 0; i < positions.length && i < this.pathPoints.length; i++) {
        if (this.pathPoints[i]) {
          this.pathPoints[i].getTransform().setWorldPosition(positions[i]);
          print(`Line3D: Updated point ${i} to (${positions[i].x.toFixed(2)}, ${positions[i].y.toFixed(2)}, ${positions[i].z.toFixed(2)})`);
        }
      }
    }

    // Regenerate the mesh with new positions
    print("Line3D: Regenerating mesh...");
    this.generateMesh();
  }

  /**
   * Get the current raw path positions as an array of vec3 (for external access)
   */
  public getCurrentPathPositions(): vec3[] {
